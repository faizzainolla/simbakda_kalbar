	   ...
	   $('#kdskpd').combogrid({  
		...
		$('#giat').combogrid({
            ...
            onSelect:function(rowIndex,rowData){                                                             
				...
				var cunit  = $('#uskpd').combogrid('getValue'); 
				$('#kd').combogrid({url:'<?php echo base_url(); ?>index.php/bhp/ambil_brg_keluar',queryParams:({skpd:cuskpd,giat:ckd,unit:cunit}) });
	...
	function load_stok(){
        ...
        var i = 0; 
         $.ajax({
                ...
                data: ({no:nomor,kode:kode,kodegiat:giat,kodeunit:unit}),
				dataType:"json", 
                success:function(data){                                          
					$.each(data,function(i,n){ 
                        nma      = n['nama'];                    
