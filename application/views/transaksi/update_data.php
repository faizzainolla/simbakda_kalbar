	...
	function data_realisasi(){
		...
		$(function(){      
		 $.ajax({
			...
			data: ({nomor:'1'}),
			dataType:"json",
			url:'<?php echo base_url(); ?>index.php/transaksi/proses_update_data_realisasi/'+nbulan,
	...
	function data_kapitalisasi(){
		...
		$(function(){      
		 $.ajax({
			...
			data: ({nomor:'1'}),
			dataType:"json",
			url:"<?php echo base_url(); ?>index.php/transaksi/proses_update_data_kapitalisasi",
			success:function(data){
			if (data = 1){
					alert('PROSES SELESAI');
					document.getElementById('load').style.visibility='hidden';
