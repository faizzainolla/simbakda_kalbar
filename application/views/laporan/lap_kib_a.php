...
<script type="text/javascript">
	...
	function openWindow($cek){
		...
		var url		= "<?php echo site_url(); ?>/laporan/lap_kib_a";	
		...
		iz = '?cbid='+cbid+'&cskpd='+cskpd+'&cnmskpd='+cnmskpd+'&cnm_bid='+cnm_bid+'&lctahu='+lctahu+'&lcbend='+lcbend+'&cnmbend='+cnmbend+'&cnmtahu='+cnmtahu+'&kondisi='+ckondisi+'&milik='+cmilik+'&tahun='+ctahun+'&tgl_reg='+tgl_reg+'&lctgl2='+lctgl2+'&pnilai='+pnilai+'&ini='+cini+'&fa='+cpilih;
   ...
   /*   $('#kondisi').combogrid({  
	 ...
	 $('#milik').combogrid({  
       ...
       columns:[[  
		...
		/* oto  = '<?php echo ($this->session->userdata('otori'));?>';
		skp  = '<?php echo ($this->session->userdata('skpd'));?>';
		alert(skp);
		if(oto=='01'){
		$("#admin_side").show();
		}if(skp=='1.02.01.00'){
...
<div id="content1"> 
    ...
    KARTU INVENTARIS BARANG TANAH</b></h3>
    <fieldset>
