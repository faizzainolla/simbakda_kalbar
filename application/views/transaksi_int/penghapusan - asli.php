...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $('#trd_hapus').edatagrid({    		   
         ...
         $('#uskpd').combogrid({  
            ...
            columns:[[  
               {field:'kd_skpd',title:'Kode SKPD',width:80},  
   ...
   function loadDetail(){
    ...
    $.ajax({
        ...
        data: ({skpd:cuskpd,nomor:cno_urut,unit:unit}),
        dataType:"json",
        success:function(data){                                          
        $.each(data,function(i,n){  
 ...
 function load_save(){ 
    ...
    if(r==true){
	  ...
	  function hapus_usulan(){
        ...
        $(document).ready(function(){
         $.post(
		 urll,({tabel:'trh_penghapusan',cnid:no_hapus,cid:'no_hapus',skpd:kd_skpd,unit:kd_unit}),
		 function(data){
            status = data;
            if (status=='0'){
                alert('Gagal Hapus..!!');
...
<div id="dialog-modal" title="PILIH BARANG YANG AKAN DIHAPUS">
    ...
    <fieldset>
        <table align="center">
            <tr>
                <td><a id="usul" class="easyui-linkbutton" iconCls="icon-ok" plain="true" onclick="javascript:load_save();">USULKAN PENGHAPUSAN</a>
