 ...
 $(document).ready(function(){
    ...
    $(function(){         
        ...
        onDblClickRow:function(rowIndex,rowData){
                ...
                nmkode  = rowData.nm_uskpd;
                tahun   = rowData.tahun;
                get(nomor,tgl,unit,kode,nmkode,tahun);
        }
	...
	//ini dari tabel trh_treatbrg 
        var i = 0;
        var nomor = document.getElementById('nomor').value; 
        var tgl   = $('#tanggal').datebox('getValue');
        var kode  = $('#uskpd').combogrid('getValue'); 
		// ------------
		// ini ngambil dari tabel trd_treatbrg sesuai nomor trh_treatbrg 
         $.ajax({
                type: "POST",
    ...
    function set_grid(){
         $('#trd').edatagrid({
              columns:[[
                    ...
                    {field:'jumlah',title:'biaya pelihara',width:100,align:"right"},
    ...
    function append_save(){
       ...
       // var nmrek  = document.getElementById('nmrek').value;
        ...
        if(tot!=0){
            ...
            $('#trd').edatagrid('appendRow',{idx:pidx,no_dokumen:no,kd_brg:kd,nm_brg:nm,jumlah:jml,harga:rg,total:to,ket:ket,satuan:satuan});
            $('#trd2').edatagrid('appendRow',{idx:pidx,no_dokumen:no,kd_brg:kd,nm_brg:nm,jumlah:jml,harga:rg,total:to,ket:ket,satuan:satuan});
...
//        var kdrek5 = document.getElementById('kdrekk5').value;
		...
		*/
	    var tott = 0;
			lcquery = "update trd_treatbrg set merek='"+mrk+"',jumlah='"+jml+"',harga='"+hrg+"',total='"+total+"',ket='"+ket+"',satuan='"+satuan+"' where kd_unit='"+unit+"' and kd_uskpd='"+skp+"' and no_dokumen='"+no+"' and kd_brg ='"+kd+"'";          
			   $.ajax({
					type: 'POST',
     ...
     function simpan(){
        ...
        $(document).ready(function(){
            $.ajax({
                type: "POST",       
                dataType : 'json',         
                data: ({tabel:'trh_treatbrg',no:cno,tgl:ctgl,uskpd:cuskpd,lokasi:cmlokasi,nmuskpd:cnmuskpd,tahun:cthn,total:ctotal}),
    ...
    function simpan_detail_dh(){
        ...
        var rows = $('#trd').datagrid('getSelections'); 
          ...
          //      kdrek5      =rows[i].kd_rek5;
                ...
                if(i>0){
                    csql = csql+","+"('"+no+"','"+kd_brg+"','"+cmlokasi+"','"+cuskpd+"','"+nm_brg+"','"+jumlah+"','"+harga+"','"+total+"','"+ket+"','"+satuan+"')";
                } else {

                    csql = "values('"+no+"','"+kd_brg+"','"+cmlokasi+"','"+cuskpd+"','"+nm_brg+"','"+jumlah+"','"+harga+"','"+total+"','"+ket+"','"+satuan+"')";                                            
                } 
        }  
            $.ajax({
                    type: 'POST',
                    data: ({tabel:'trd_treatbrg',sql:csql,no:no,unit:cmlokasi}),
                    url:"<?php echo base_url(); ?>index.php/transaksi/simpan_treatbrg",
                    success:function(data){ 
                        status = data.pesan;
                    }
     ...
     function hapus(){
      ...
      var del   = confirm('Apakah Anda Yakin Ingin Menghapus Data Terpilih?');
    if(del==true){
...
<div id="tabs" >
		<p><h3 align="center">INPUT RENCANA KEBUTUHAN PEMELIHARAAN BARANG UNIT (RKPBU)</h3></p>
        ...
        <table>   
		   ...
		   <tr>
            ...
            <tr>
                <td></td>
                <td></td>
                <td><input id="merek" hidden name="merek" value=""/>  </td>            
            </tr>     
            <tr>
                <td>Biaya Pelihara</td>
                <td>:</td>
            ...
            <tr>
                <td>Harga Barang</td>
                <td>:</td>
                <td><input id="hrg" disabled name="hrg" value="" style="text-align: right;" onkeyup="hitung();"/>  </td>            
            </tr> 
            <tr>
                <td>Total Harga</td>
                <td>:</td>
