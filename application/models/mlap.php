	...
	if($this->auth->is_logged_in() == false){
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
            ...
            <tr>
             ...
             $csql = "SELECT a.nm_brg,a.merek,a.jumlah,a.jumlah AS jml,a.harga AS nil,(SELECT ISNULL((a.harga),0)) AS harga,a.kd_brg,b.kd_rek5,a.ket
	...
	if($this->auth->is_logged_in() == false){
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
			...
			<tr>
             ...
             $csql = "SELECT a.tgl_periksa,a.kd_uskpd AS dari,a.no_faktur,a.tgl_faktur,b.nm_brg,
			 ISNULL((b.jumlah),0) as jumlah,ISNULL(sum(b.jumlah),0) as jmlx,b.harga,ISNULL((b.harga),0)as harga,
			 ISNULL((b.total),0)as total,ISNULL(sum(b.total),0) as totalx,a.no_bap,a.tgl_bap,a.keterangan       
                    FROM trh_terimabrg a INNER JOIN trd_terimabrg b ON a.no_bap=b.no_bap WHERE a.kd_uskpd = '$cskpd'
					GROUP BY a.tgl_periksa,a.kd_uskpd,a.no_faktur,a.tgl_faktur,b.nm_brg,a.no_bap,a.tgl_bap,a.keterangan,b.harga,b.total,b.jumlah";
			 ...
			 $totalxx=0;
             foreach ($hasil->result() as $row){
			 $totalxx = $totalxx+$row->totalx;
			 $jumlahxx = $jumlahxx+$row->jmlx;
                $i++;    
                $cRet .="
                    <tr>
	...
	if($this->auth->is_logged_in() == false){
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
            ...
            <tr>
             ...
             $csql = "SELECT a.tgl_bak,a.no_bak,b.nm_brg,ISNULL((b.jumlah),0) as jumlah,sum(b.jumlah) as jml,ISNULL((b.harga),0) as harga,sum(b.harga) as biaya,ISNULL((b.total),0) as total,b.ket AS untuk,(a.tgl_bak) AS tgl_penyerahan,b.ket   
                 FROM trh_keluarbrg a INNER JOIN trd_keluarbrg b ON a.no_bak = b.no_bak WHERE a.kd_uskpd = '$cskpd'
				 GROUP BY a.tgl_bak,	a.no_bak,	b.nm_brg,b.jumlah, b.harga,b.total,b.ket,a.tgl_bak";
             ...
             foreach ($hasil->result() as $row)
             {
			 $biayaxx	= $row->biaya+$biayaxx;
			 $jumlahxx	= $row->jml+$jumlahxx;
...
a.no_reg,
a.merek,a.kd_brg,a.nilai,(CASE WHEN a.kondisi ='B' THEN 'B' ELSE '-' END) as kondisia,
(CASE WHEN a.kondisi ='KB' THEN 'KB' ELSE '-' END) as kondisib,
(CASE WHEN a.kondisi ='RB' THEN 'RB' ELSE '-' END) as kondisic,
...
a.no_reg,
a.judul AS merek,a.kd_brg,a.nilai,(CASE WHEN a.kondisi ='B' THEN 'B' ELSE '-' END) as kondisia,
(CASE WHEN a.kondisi ='KB' THEN 'KB' ELSE '-' END) as kondisib,
(CASE WHEN a.kondisi ='RB' THEN 'RB' ELSE '-' END) as kondisic,
...
$csql = "SELECT * FROM 
			...
			b.nm_brg,'' AS merek,a.no_sertifikat AS gabung,'' AS kd_bahan,a.no_urut,0 as nil_kap, 
a.asal,a.tahun,CONVERT(varchar(10), a.luas) AS silinder,'' kd_satuan,a.kondisi,
COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
group by a.kd_brg,a.nilai,a.keterangan,a.asal,a.tahun,a.no_reg,b.nm_brg,a.no_sertifikat,a.kondisi,a.luas,a.no_urut
			UNION
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,CONVERT(varchar(10), a.merek) as merek,a.pabrik+'/'+no_rangka+'/'+no_mesin AS gabung,a.kd_bahan,a.no_urut,
0 as nil_kap, a.asal,a.tahun,CONVERT(varchar(10), a.silinder) as silinder,a.kd_satuan,a.kondisi,
COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
			...
			AND (a.no_pindah IS NULL OR a.no_pindah='' OR a.tgl_pindah>='$sampai_tgl') 
			AND (a.no_hapus IS NULL OR a.no_hapus='' OR a.tgl_hapus>='$sampai_tgl')  
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9') and a.nilai<>'0' and a.milik='12' and kondisi!='RB'
			GROUP BY a.kd_brg,a.nilai,a.asal, a.tahun,a.silinder,a.kd_satuan,
a.kondisi,a.pabrik,a.no_rangka,a.no_polisi,a.no_mesin,a.keterangan,
a.no_reg,b.nm_brg,a.merek,kd_bahan,no_urut 
			UNION
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,CONVERT(varchar(10), a.luas_tanah) AS merek,a.no_dok AS gabung,a.jenis_gedung AS kd_bahan,a.no_urut, 
(SELECT (CASE WHEN 1='1' THEN SUM(nilai)ELSE SUM(total) END) FROM trkib_c_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='2017-4-20' ) as nil_kap,
 a.asal,a.tahun,CONVERT(varchar(10), a.konstruksi) AS silinder,'' AS kd_satuan,
a.kondisi,COUNT(a.nilai) AS jumlah,
ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
no_reg,nm_brg,luas_tanah,no_dok,jenis_gedung,no_urut, a.asal,a.tahun,a.konstruksi,kondisi,kd_skpd,kd_unit,id_barang
 
			UNION
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,CONVERT(varchar(10), a.panjang) AS merek,CONVERT(varchar(10), a.luas) AS gabung,'' AS kd_bahan,a.no_urut,
(SELECT (CASE WHEN 1='1' THEN SUM(nilai)ELSE SUM(total) END) FROM trkib_d_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='2017-4-20' ) as nil_kap,
 a.asal,a.tahun,CONVERT(varchar(10), a.lebar) AS silinder,'' AS kd_satuan,a.kondisi,
COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
no_reg,nm_brg,panjang,luas,no_urut,kd_skpd,kd_unit,id_barang,asal,tahun,lebar,kondisi
			UNION
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,a.judul AS merek,a.spesifikasi AS gabung,a.kd_bahan,a.no_urut,0 as nil_kap, a.peroleh AS asal,
a.tahun,a.tipe AS silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,
a.keterangan 
			FROM trkib_e a left join mbarang b on a.kd_brg=b.kd_brg WHERE a.kd_skpd='$cskpd' $th 
			GROUP BY a.kd_brg,a.tahun,a.nilai,a.keterangan,no_reg,nm_brg,a.judul,spesifikasi,kd_bahan,no_urut,peroleh,
a.tahun,a.tipe,a.kd_satuan,a.kondisi
			UNION
			
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,'' AS merek,CONVERT(varchar(10), a.luas) AS gabung,'' AS kd_bahan,a.no_urut,0 as nil_kap, a.asal,a.tahun,
'' AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,
a.keterangan 
...
no_reg,luas, no_urut, asal,kondisi,nm_brg
			UNION
			SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,'' AS merek,CONVERT(varchar(10), a.luas) AS gabung,'' AS kd_bahan,a.no_urut,0 as nil_kap, a.asal,a.tahun,
...
a.keterangan 
			...
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9') and a.nilai<>'0' and a.milik='12'
	group by a.kd_brg,a.tahun,a.nilai,a.keterangan,
no_reg,nm_brg,luas,no_urut,asal,kondisi
...
a.asal,a.tahun,a.luas AS silinder,'' kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
a.status_tanah,a.tgl_sertifikat,a.no_sertifikat,a.penggunaan,a.asal,a.nilai,a.keterangan ,no_reg,no_urut,asal,kondisi
		ORDER BY tahun,kd_brg,no_reg";
	}elseif($jenis=='3'){
		$csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,b.nm_brg,a.merek,
		a.pabrik+'/'+no_rangka+'/'+no_mesin AS gabung,a.kd_bahan,a.no_urut,'' as nil_kap,
a.asal,a.tahun,a.silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
FROM trkib_b a left join mbarang b on a.kd_brg=b.kd_brg WHERE a.kd_skpd='$cskpd' AND a.tgl_reg<='$sampai_tgl' $th and a.nilai<>'0' and a.milik !='12' 
			...
			AND (a.no_pindah IS NULL OR a.no_pindah='' OR a.tgl_pindah>='$sampai_tgl') 
			AND (a.no_hapus IS NULL OR a.no_hapus='' OR a.tgl_hapus>='$sampai_tgl')  
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9') and kondisi!='RB'
GROUP BY a.kd_brg,a.nilai,a.asal, a.tahun,a.silinder,a.kd_satuan,
a.kondisi,a.pabrik,a.no_rangka,a.no_polisi,a.no_mesin,a.keterangan,
a.no_reg,b.nm_brg,a.merek,kd_bahan,no_urut
			ORDER BY tahun,kd_brg,no_reg";
	}elseif($jenis=='4'){
		$csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,b.nm_brg,a.luas_tanah AS merek,a.no_dok AS gabung,a.jenis_gedung AS kd_bahan,a.no_urut,'' as nil_kap,
			(SELECT (CASE WHEN 1='1' THEN SUM(nilai)ELSE SUM(total) END) FROM trkib_c_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='$sampai_tgl') as nil_kap,
a.asal,a.tahun,a.konstruksi AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
no_reg,nm_brg,luas_tanah,no_dok,jenis_gedung,no_urut, a.asal,a.tahun,a.konstruksi,kondisi,kd_skpd,kd_unit,id_barang
		...
		$csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,b.nm_brg,a.panjang AS merek,a.luas AS gabung,'' AS kd_bahan,a.no_urut,
			(SELECT (CASE WHEN 1='1' THEN SUM(nilai)ELSE SUM(total) END)  FROM trkib_d_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit 
			and a.id_barang=id_barang and tgl_reg<='$sampai_tgl') as nil_kap,
a.asal,a.tahun,a.lebar AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
no_reg,nm_brg,panjang,luas,no_urut,kd_skpd,kd_unit,id_barang,asal,tahun,lebar,kondisi
		...
		$csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,b.nm_brg,a.judul AS merek,a.spesifikasi AS gabung,a.kd_bahan,a.no_urut,'' as nil_kap,
a.peroleh AS asal,a.tahun,a.tipe AS silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
FROM trkib_e a left join mbarang b on a.kd_brg=b.kd_brg WHERE a.kd_skpd='$cskpd' AND a.tgl_reg<='$sampai_tgl' $th and a.nilai<>'0' and a.milik !='12'   and kondisi!='RB'
GROUP BY a.kd_brg,a.tahun,a.nilai,a.keterangan,no_reg,nm_brg,a.judul,spesifikasi,kd_bahan,no_urut,peroleh,
a.tahun,a.tipe,a.kd_satuan,a.kondisi";
	}elseif($jenis=='7'){
		$csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,b.nm_brg,'' AS merek,a.luas AS gabung,'' AS kd_bahan,a.no_urut,'' as nil_kap,
a.asal,a.tahun,'' AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
a.asal,a.tahun,a.luas AS silinder,'' kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
...
a.asal,a.tahun,a.silinder,a.kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
...
SELECT a.kd_brg,IF(MAX(a.no_reg)=MIN(a.no_reg),a.no_reg,CONCAT(MIN(a.no_reg),'-',MAX(a.no_reg)))AS no_reg,b.nm_brg,a.luas_tanah AS merek,a.no_dok AS gabung,a.jenis_gedung AS kd_bahan,a.no_urut,
			...
			and a.id_barang=id_barang and tgl_reg<='$sampai_tgl') as nil_kap,
a.asal,a.tahun,a.konstruksi AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
...
SELECT a.kd_brg,IF(MAX(a.no_reg)=MIN(a.no_reg),a.no_reg,CONCAT(MIN(a.no_reg),'-',MAX(a.no_reg)))AS no_reg,b.nm_brg,a.panjang AS merek,a.luas AS gabung,'' AS kd_bahan,a.no_urut,
			...
			and a.id_barang=id_barang and tgl_reg<='$sampai_tgl') as nil_kap,
a.asal,a.tahun,a.lebar AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
...
a.peroleh AS asal,a.tahun,a.tipe AS silinder,a.kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
...
a.asal,a.tahun,'' AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
 ...
 */	$this->fpdf->Setx(273);
	...
	a.asal,a.tahun,a.silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,sum(nilai) as nilai2,count(nilai) as jumlah2,LEFT(RTRIM(a.keterangan),25) AS keterangan
	...
	a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,SUM(nilai) AS nilai3,COUNT(nilai) AS jumlah3,LEFT(RTRIM(a.keterangan),25) AS keterangan
	...
	a.peroleh AS asal,a.tahun,a.tipe AS silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,sum(nilai) as nilai5,count(nilai) as jumlah5,LEFT(RTRIM(a.keterangan),25) AS keterangan 
...
(SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,a.merek,a.pabrik+'/'+no_rangka+'/'+no_mesin AS gabung,a.kd_bahan,a.no_urut,
a.asal,a.tahun,a.silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
FROM trkib_b a left join mbarang b on a.kd_brg=b.kd_brg where a.tgl_reg<='$sampai_tgl' $skpd AND (a.kondisi='RB' or a.kondisi='HB' or a.kondisi='PM') 
			...
			OR a.kd_riwayat='9') 
			GROUP BY a.kd_brg,a.tahun,a.nilai,a.keterangan,
				no_reg,b.nm_brg,a.merek,pabrik,no_rangka,no_mesin,kd_bahan,no_urut,asal,silinder,kd_satuan,
			kondisi
UNION
SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END) AS no_reg,
...
FROM trkib_c a left join mbarang b on a.kd_brg=b.kd_brg where a.tgl_reg<='$sampai_tgl' $skpd AND (a.kondisi='RB' or a.kondisi='HB' or a.kondisi='PM') 
	...
	no_reg,b.nm_brg,a.luas_tanah,no_dok,jenis_gedung,no_urut,asal,tahun,konstruksi,kondisi 
UNION
SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END) AS no_reg,
b.nm_brg,
...
AND (a.kondisi='RB' or a.kondisi='HB' or a.kondisi='PM')
			...
			AND (a.no_pindah IS NULL OR a.no_pindah='' OR a.tgl_pindah>='$sampai_tgl') 
			AND (a.no_hapus IS NULL OR a.no_hapus='' OR a.tgl_hapus>='$sampai_tgl')  
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9')GROUP BY a.kd_unit,b.nm_brg,a.kd_brg,a.tahun,a.kondisi,a.konstruksi,a.panjang,a.luas,a.lebar,a.alamat1,
a.kd_tanah,a.tgl_dok,a.no_dok,a.asal,a.nilai,no_reg,no_urut,keterangan

UNION
SELECT a.kd_brg,
(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END) AS no_reg,
b.nm_brg,a.judul AS merek,a.spesifikasi AS gabung,a.kd_bahan,a.no_urut,
a.peroleh AS asal,a.tahun,a.tipe AS silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
...
no_reg,judul,spesifikasi,no_urut,peroleh,tahun,kd_satuan,kondisi
 
 ) faiz  ORDER BY kd_brg,no_reg";
 			 }else{
				  if($jenis=='2'){
				 $csql = "SELECT a.kd_brg,(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
b.nm_brg,a.merek,a.pabrik+'/'+no_rangka+'/'+no_mesin AS gabung,a.kd_bahan,a.no_urut,
a.asal,a.tahun,a.silinder,a.kd_satuan,a.kondisi,COUNT(a.nilai) AS jumlah,ISNULL(SUM(a.nilai),0) AS nilai,a.keterangan 
FROM trkib_b a left join mbarang b on a.kd_brg=b.kd_brg where a.tgl_reg<='$sampai_tgl' $skpd AND (a.kondisi='RB' or a.kondisi='HB' or a.kondisi='PM') 
			AND (a.no_mutasi IS NULL OR a.no_mutasi='' OR a.tgl_mutasi>='$sampai_tgl') 
...
ISNULL(SUM(a.total),0) AS nilai,a.keterangan 
FROM trkib_d a LEFT JOIN mbarang b ON a.kd_brg=b.kd_brg where a.tgl_reg<='$sampai_tgl' $skpd 
AND (a.kondisi='RB' or a.kondisi='HB' or a.kondisi='PM')
...
b.nm_brg,CONVERT(varchar(10), a.luas_tanah) AS merek,a.no_dok AS gabung,a.jenis_gedung AS kd_bahan,a.no_urut,
	...
	no_reg,b.nm_brg,a.luas_tanah,no_dok,jenis_gedung,no_urut,asal,tahun,konstruksi,kondisi
			...
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9')
				...
				UNION
				SELECT a.kd_brg,IF(MAX(a.no_reg)=MIN(a.no_reg),a.no_reg,CONCAT(MIN(a.no_reg),'-',MAX(a.no_reg)))AS no_reg,b.nm_brg,a.luas_tanah AS merek,a.no_dok AS gabung,a.jenis_gedung AS kd_bahan,a.no_urut,
				a.asal,a.tahun,a.konstruksi AS silinder,'' AS kd_satuan,a.kondisi,COUNT(a.total) AS jumlah,ISNULL(SUM(a.total),0)+ISNULL(SUM(c.total),0) AS nilai,a.keterangan 
				FROM trkib_c a left join mbarang b on a.kd_brg=b.kd_brg LEFT JOIN trkib_c_kap c ON a.id_barang=c.id_barang and c.tgl_reg<='$sampai_tgl' WHERE a.tgl_reg<='$sampai_tgl' $skpd $th and (a.total<10000000 or a.kd_riwayat='1') and a.total <>'0' and a.kondisi<>'RB' 
			AND (a.no_mutasi IS NULL OR a.no_mutasi='' OR a.tgl_mutasi>='$sampai_tgl') 
			AND (a.no_pindah IS NULL OR a.no_pindah='' OR a.tgl_pindah>='$sampai_tgl') 
			AND (a.no_hapus IS NULL OR a.no_hapus='' OR a.tgl_hapus>='$sampai_tgl')  
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' OR a.kd_riwayat='9')
 ...
 */	$this->fpdf->Setx(273);
	...
	$sql="SELECT TOP 1 kd_skpd,nm_lokasi,kd_lokasi from mlokasi 
	$where group by kd_skpd,nm_lokasi,kd_lokasi";	
	...
	foreach ($hsl1->result() as $row)
			...
			$sql2="SELECT ISNULL(SUM(a.total),0)+ISNULL(SUM(c.total),0) as nilai2,count(a.total) as jumlah2 FROM trkib_c a left join mbarang b on a.kd_brg=b.kd_brg LEFT JOIN trkib_c_kap c ON a.id_barang=c.id_barang
			and c.tgl_reg<='$sampai_tgl' where a.tgl_reg<='$sampai_tgl' $where1 $th 
			and (a.total<10000000 or a.kd_riwayat='1') and a.total <>'0' and a.kondisi<>'RB' 
			AND (a.no_mutasi IS NULL OR a.no_mutasi='' OR a.tgl_mutasi>='$sampai_tgl') 
			AND (a.no_pindah IS NULL OR a.no_pindah='' OR a.tgl_pindah>='$sampai_tgl') 
			AND (a.no_hapus IS NULL OR a.no_hapus='' OR a.tgl_hapus>='$sampai_tgl')  
			AND (a.tgl_riwayat>'$sampai_tgl' OR a.kd_riwayat IS NULL OR a.kd_riwayat='' 
...
/* 		if($oto='01'){
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
           ...
           $cRet .= "<tr>
			...
			$sql="SELECT TOP 1 kd_skpd,nm_lokasi,kd_lokasi from mlokasi $where group by kd_skpd,kd_lokasi,nm_lokasi";	
	$hasil = $this->db->query($sql);	
	$i=1;
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">			
            ...
            if($pnilai=='1'){
             $csql = "SELECT kd_brg,no_reg,nm_brg,merek,spek,bahan,asal,tahun,ukuran,satuan,kondisi,jml_awal,
						...
						CASE WHEN ((a.tgl_mutasi BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_pindah BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_hapus BETWEEN '$tgl_awal' AND '$tgl_akhir')  
						OR (a.tgl_riwayat BETWEEN '$tgl_awal' AND '$tgl_akhir') ) THEN COUNT(a.nilai) ELSE 0 END AS jml_kurang,
						CASE WHEN ((a.tgl_mutasi BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_pindah BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_hapus BETWEEN '$tgl_awal' AND '$tgl_akhir')  
						OR (a.tgl_riwayat BETWEEN '$tgl_awal' AND '$tgl_akhir') ) THEN SUM(a.nilai) ELSE 0 END AS kurang,
						CASE WHEN ((a.tgl_reg BETWEEN '$tgl_awal' AND '$tgl_akhir') ) THEN COUNT(a.nilai) ELSE 0 END AS jml_tambah,
						CASE WHEN ((a.tgl_reg BETWEEN '$tgl_awal' AND '$tgl_akhir') ) THEN SUM(a.nilai) ELSE 0 END AS tambah,
						CASE WHEN ((a.tgl_reg<='$tgl_akhir') ) THEN COUNT(a.nilai) ELSE 0 END AS jml_akhir,
						CASE WHEN ((a.tgl_reg<='$tgl_akhir') ) THEN SUM(a.nilai) ELSE 0 END AS akhir,a.keterangan
						FROM trkib_a a 
						...
						a.no_sertifikat,a.penggunaan,a.asal,a.nilai ,no_reg,kondisi,
						tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
						UNION
						SELECT a.kd_brg,
						(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
						b.nm_brg,a.merek,a.no_mesin+'/'+a.no_polisi AS spek,
						...
						GROUP BY a.kd_unit,b.nm_brg,a.kd_brg,a.merek,
						a.silinder,a.tahun,a.kd_warna,a.kd_bahan,a.asal,a.pabrik,a.no_rangka,
						a.no_mesin,a.no_polisi,a.no_bpkb,a.nilai,no_reg,kd_satuan,kondisi,
						tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
						UNION
						SELECT a.kd_brg,
						(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
						b.nm_brg,'-' AS merek,'-' AS spek,
						CASE WHEN a.konstruksi ='beton' THEN konstruksi ELSE '-' END AS bahan,a.asal,a.tahun,a.konstruksi2 AS ukuran,
						...
						GROUP BY a.kd_unit,b.nm_brg,a.kd_brg,a.tahun,a.kondisi,a.konstruksi,a.jenis_gedung,
						a.luas_lantai,a.alamat1,a.kd_tanah,a.tgl_dok,a.no_dok,a.luas_tanah,a.status_tanah,a.asal,a.nilai,
						no_reg,konstruksi2,tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
						UNION
						SELECT a.kd_brg,
						(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
						b.nm_brg,'-' AS merek,'-' AS spek,
						a.konstruksi AS bahan,a.asal,a.tahun,CONVERT(varchar(10), a.panjang)+'/'+CONVERT(varchar(10), a.luas)+'/'+CONVERT(varchar(10), a.lebar) AS ukuran,
						'-' AS satuan,a.kondisi,(0) AS jml_awal,(0) AS awal,
						CASE WHEN ((a.tgl_mutasi BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_pindah BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_hapus BETWEEN '$tgl_awal' AND '$tgl_akhir')  
						...
						no_reg,asal,tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
						UNION
						SELECT a.kd_brg,
						(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
						...
						OR a.kd_riwayat BETWEEN '$tgl_awal' AND '$tgl_akhir') and a.nilai<>'0'
						GROUP BY a.kd_unit,b.nm_brg,a.kd_brg,a.kd_bahan,a.jenis,a.tipe,a.nilai,a.keterangan,
						no_reg,judul,spesifikasi,asal,tahun,kd_satuan,kondisi,
						tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
						UNION
						SELECT a.kd_brg,
						(CASE WHEN MAX(a.no_reg)=MIN(a.no_reg) THEN a.no_reg ELSE MIN(a.no_reg)+'-'+MAX(a.no_reg) END)AS no_reg,
						b.nm_brg,'' AS merek,a.kd_tanah AS spek,
						CASE WHEN jenis='beton' THEN jenis ELSE '-' END AS bahan,a.asal,a.tahun,CONVERT(varchar(10), a.luas) AS ukuran,
						'-' AS satuan,a.kondisi,(0) AS jml_awal,(0) AS awal,
						CASE WHEN ((a.tgl_mutasi BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_pindah BETWEEN '$tgl_awal' AND '$tgl_akhir') 
						OR (a.tgl_hapus BETWEEN '$tgl_awal' AND '$tgl_akhir')  
						...
						no_reg,luas,kondisi,tgl_mutasi,tgl_pindah,tgl_hapus,tgl_riwayat,tgl_reg,keterangan
...
$cnama	= $row->nama;
		...
		if ($ckodex=='01'){
		  ...
		  foreach ($hasil->result() as $row){
			...
			$csql = "SELECt ISNULL (sum(nilai),0) as nilai FROM trkib_a WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($csql);
			foreach ($hasil->result() as $row){
            $charga2 = $row->nilai;
			}					
		...
		if ($ckodex=='02'){
		  ...
		  foreach ($hasil->result() as $row){
			...
			$ccsql = "SELECT ISNULL (sum(nilai),0) as nilai FROM trkib_b WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($ccsql);
			foreach ($hasil->result() as $row){
            $charga2 = $row->nilai;
			}
		...
		if ($ckodex=='03'){
			...
			$csql = "SELECt ISNULL (sum(nilai),0) as nilai FROM trkib_c WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($csql);
			...
			$csql = "SELECt ISNULL (sum(nilai),0) as nilai FROM trkib_d WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($csql);
			...
			$csql = "SELECt ISNULL (sum(nilai),0) as nilai FROM trkib_e WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($csql);
			...
			$csql = "SELECt ISNULL (sum(nilai),0) as nilai FROM trkib_f WHERE kd_unit='$cbid' and left(kd_brg,4)='$ckode2'";
			$hasil = $this->db->query($csql);
...
(SELECT ISNULL((SELECT  COUNT(b.jumlah) FROM trkib_a b WHERE a.kd_unit=b.kd_unit AND b.id_barang=a.id_barang AND  b.tgl_reg<'$tgl_awal'),0)) AS jumlah_awal,
(SELECT ISNULL((SELECT  SUM(b.nilai) FROM trkib_a b WHERE a.kd_unit=b.kd_unit AND b.id_barang=a.id_barang AND  b.tgl_reg<'$tgl_awal'),0)) AS nilai_awal,
...
WHERE c.kd_skpd='$cskpd' 
    ...
    $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
			...
			<tr>
                <td width=\"10%\"align=\"left\" style=\"font-size:14px; font-family:tahoma;\"></td>
				<td width =\"80%\" align=\"center\" style=\"font-size:14px; font-family:tahoma;\"><b>DAFTAR RIWAYAT ASET</b></td>
				<td width=\"10%\"align=\"left\" style=\"font-size:12px; font-family:tahoma;\"></td>
            </tr>
            </table><BR/><BR/>
	...
	if($this->auth->is_logged_in() == false){
	 ...
	 $ckib="trkib_f";
        ...
        $cRet .= "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"2\" cellpadding=\"1\">";
			$csql = "SELECT a.milik+'.'+a.wilayah+'.'+LEFT(c.kd_uker,5)+'.'+RIGHT(a.tahun,2)+'.',RIGHT(c.kd_lokasi,5) AS id_barang,
			a.kd_brg+'.'+right(a.no_reg,4) as no,b.nm_brg,a.detail_brg FROM $ckib a
			LEFT JOIN mbarang b ON a.kd_brg=b.kd_brg 
...
$csql="SELECT a.kd_brg AS kode,b.nm_brg,a.merek,YEAR(a.tgl_oleh),
			(CASE WHEN $pnilai='1' THEN $fnilai ELSE a.total END ) as nilai, RTRIM(c.umur) AS umur, 
			(CASE WHEN YEAR(a.tgl_oleh)='$tahun' THEN 0 ELSE ($tahun-YEAR(a.tgl_oleh)) END) AS th_lalu,
			$tahun AS th_ini,YEAR(a.tgl_oleh) as tahun, 
			($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun,
			(CASE WHEN YEAR(a.tgl_oleh)='$tahun' THEN 0 ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-YEAR(a.tgl_oleh)))=1 THEN ($fnilai-($fnilai/RTRIM(c.umur))) WHEN (RTRIM(c.umur)-($tahun-YEAR(a.tgl_oleh)))<=0 THEN $fnilai ELSE ($tahun-YEAR(a.tgl_oleh))*($fnilai/RTRIM(c.umur)) END ) END) AS tot_th_belum,
			(CASE WHEN YEAR(a.tgl_oleh)='$tahun' THEN ($fnilai/RTRIM(c.umur)) ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-YEAR(a.tgl_oleh)))=1 THEN ($fnilai-($fnilai/RTRIM(c.umur))) WHEN (RTRIM(c.umur)-($tahun-YEAR(a.tgl_oleh)))<=0 THEN 0 ELSE ($tahun-YEAR(a.tgl_oleh))*($fnilai/RTRIM(c.umur)) END ) END) AS nil_th_ini
		FROM trkib_b a
        ...
        <table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
           <thead>
            <tr>
                ...
                <td colspan=\"7\" align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>PENYUSUTAN</b></td>
                <td rowspan=\"2\" align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>NILAI BUKU</b></td>
			</tr>
			<tr>	
				<!--td align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>Nilai Residu</b></td-->
                <td align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>Masa Manfaat</b></td>
				<td align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>Tahun Lalu</b></td>
				<td align=\"center\" bgcolor=\"#ADFF2F\" style=\"font-size:10px; font-family:tahoma;\"><b>Penambahan Masa Manfaat</b></td>
            ...
            //$pnilai ==> (2)nilai baru || nilai lama(1)
			//$fnilai ==> (2) total 	|| nilai (1)
			if($tahun<=2015){
				$csql ="
				SELECT a.id_barang,a.kd_brg AS kode,b.nm_brg,
'' as merek,year(a.tgl_oleh) as tahun, 
(SELECT CASE WHEN $pnilai='1' THEN SUM(nilai) ELSE SUM(total) END FROM trkib_c_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as kap,
(SELECT max(year(tgl_reg)) FROM trkib_c_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as th_kap, 
CASE WHEN $pnilai='1' THEN $fnilai ELSE a.total END as nilai,
RTRIM(c.umur) AS umur, 
CASE WHEN year(a.tgl_oleh)='$tahun'THEN 0 ELSE ($tahun-year(a.tgl_oleh)) END AS th_lalu,
$tahun AS th_ini,year(a.tgl_oleh), 
($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun,
year(a.tgl_oleh) as th,
CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 THEN ($fnilai-($fnilai/RTRIM(c.umur))) WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0 THEN $fnilai ELSE ($tahun-year(a.tgl_oleh))*($fnilai/RTRIM(c.umur)) END) END AS tot_th_belum,
CASE WHEN year(a.tgl_oleh)='$tahun' THEN ($fnilai/RTRIM(c.umur)) ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 THEN ($fnilai/RTRIM(c.umur)) WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0 THEN 0 ELSE ($fnilai/RTRIM(c.umur)) END) END AS nil_th_ini
				...
				$csql ="
				SELECT a.id_barang,a.kd_brg AS kode,b.nm_brg,'' as merek,year(a.tgl_oleh) as tahun,
				(SELECT CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END  FROM trkib_c_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit 
				...
				and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as th_kap,(SELECT SUM(tmh) tmh FROM 
				(SELECT f.id_barang,f.kd_brg,CASE WHEN d.total/f.total*100>30 AND d.total/f.total*100<=45 and f.nilai=f.total THEN 5 
				WHEN d.total/f.total*100>45 AND d.total/f.total*100<=65 and f.nilai=f.total THEN 10
				WHEN d.total/f.total*100>65 and f.nilai=f.total THEN 15
				WHEN d.total/f.total*100>=100 and f.nilai<>f.total THEN 1
				ELSE 0 END tmh FROM trkib_c_kap d RIGHT JOIN trkib_c f ON d.kd_skpd=f.kd_skpd AND d.kd_unit=f.kd_unit 
				AND d.id_barang=f.id_barang where f.kd_skpd='$kdbid' and  YEAR(d.tgl_reg)>='2016' AND d.tgl_reg<='$tahun-12-31' and (f.kondisi<>'RB' AND f.kondisi<>'HB' AND f.kondisi<>'PM') )v where v.kd_brg=a.kd_brg and v.id_barang=a.id_barang
				GROUP BY kd_brg) as ani,
				CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END as nilai,RTRIM(c.umur) AS umur,
				CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE ($tahun-year(a.tgl_oleh)) END AS th_lalu,
				$tahun AS th_ini,year(a.tgl_oleh),
				($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun,year(a.tgl_oleh) as th,

				CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE 
			    (CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 
				THEN ($fnilai-($fnilai/RTRIM(c.umur))) 
				WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0 
				THEN $fnilai ELSE($tahun-year(a.tgl_oleh))*($fnilai/RTRIM(c.umur)) END) 
				END AS tot_th_belum,

				CASE WHEN year(a.tgl_oleh)='$tahun'
				THEN ($fnilai/RTRIM(c.umur)) ELSE 
				(CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 
				THEN ($fnilai/RTRIM(c.umur)) WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0 
				THEN 0 ELSE ($fnilai/RTRIM(c.umur)) END) END AS nil_th_ini
				...
				AND kd_skpd='$kdbid' AND YEAR(a.tgl_oleh)<='$tahun' 
				AND a.tgl_oleh<='$tahun-12-31' and 
				($fnilai>=10000000 OR (a.total >=10000000 and a.kd_riwayat='9')) 
				AND (a.kondisi<>'RB' AND a.kondisi<>'HB' AND a.kondisi<>'PM')
				$lutji and a.kd_pemilik='12'
				ORDER BY year(a.tgl_reg),a.kd_brg";
			}
				...
				$vero	= 0;
				$falove	= 0;
				/*__________Kebijakan Baru 2016__________
					30% 	s.d 	45%			==>5
					<45% 	s.d 	65%			==>10
					<65% 	s.d  	seterusnya	==>15
				...
				foreach ($hasil->result() as $row){
					...
					if($row->th=='$tahun'){
						...
						$kamu	= ($aku/$row->umur);
						$tot_sst= (($buku_lalu+$row->kap)/($row->umur-$row->th_lalu+$ani));
					}else{
						$cinta	= $aku;
						$kamu	= 0;
						$tot_sst= 0;
					...
					<tr>
						...
						<td align=\"center\" width =\"10%\" style=\"font-size:10px; font-family:tahoma;\">".$row->kode."</td>
						<td align=\"left\" width =\"30%\" style=\"font-size:10px; font-family:tahoma;\"  >".$row->nm_brg."</td>
						<td align=\"left\" width =\"8%\" style=\"font-size:10px; font-family:tahoma;\"   >".$row->merek."</td>
						<td align=\"center\" width =\"7%\" style=\"font-size:10px; font-family:tahoma;\" >".$row->tahun."</td>
						<td align=\"right\" width =\"7%\" style=\"font-size:10px; font-family:tahoma;\"  >".number_format($row->nilai)."</td>
						<td align=\"right\" width =\"7%\" style=\"font-size:10px; font-family:tahoma;\"  >".number_format($aku)."</td>
						<td align=\"center\" width =\"2%\" style=\"font-size:10px; font-family:tahoma;\" >".$row->umur."</td>
						<td align=\"center\" width =\"2%\" style=\"font-size:10px; font-family:tahoma;\" >".$row->th_lalu."</td>
						<td align=\"center\" width =\"2%\" style=\"font-size:10px; font-family:tahoma;\" >".$ani."</td>
       ...
       // $pnilai		= $_REQUEST['pnilai'];
        ...
        <table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
           <thead>
			...
			if($tahun<=2015){	
			$csql ="
				SELECT a.kd_brg AS kode,b.nm_brg,'-' as merek,
year(a.tgl_oleh),
( CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END) as nilai,
RTRIM(c.umur) AS umur, 
( CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE ($tahun-year(a.tgl_oleh)) END) AS th_lalu,
$tahun AS th_ini,year(a.tgl_oleh) as tahun, 
($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun, 
(SELECT ( CASE WHEN $pnilai='1' THEN SUM(nilai) ELSE SUM(total) END ) FROM trkib_d_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as kap,
(SELECT max(year(tgl_reg)) FROM trkib_d_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as th_kap,year(a.tgl_oleh) as th,
(SELECT SUM(tmh) tmh FROM 
(SELECT f.id_barang,f.kd_brg, CASE WHEN d.total/f.total*100>P1 AND d.total/f.total*100<=P2 AND f.nilai=f.total THEN M1 WHEN d.total/f.total*100>P2 AND d.total/f.total*100<=P4 AND f.nilai=f.total THEN M2 WHEN d.total/f.total*100>P4 AND f.nilai=f.total THEN M3 WHEN d.total/f.total*100>=100 AND f.nilai<>f.total THEN 1 ELSE 0 END tmh
 FROM trkib_d_kap d RIGHT JOIN trkib_d f ON d.kd_skpd=f.kd_skpd AND d.kd_unit=f.kd_unit AND d.id_barang=f.id_barang left join mmanfaat h on left(f.kd_brg,8)=h.kd_barang where f.kd_skpd='4.00.01.01' and YEAR(d.tgl_reg)>='2016' AND d.tgl_reg<='$tahun-12-31' AND (f.kondisi<>'RB' AND f.kondisi<>'HB' AND f.kondisi<>'PM'))v where v.kd_brg=a.kd_brg and v.id_barang=a.id_barang GROUP BY kd_brg) as ani,
CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 THEN ($fnilai-($fnilai/RTRIM(c.umur))) WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0 THEN $fnilai ELSE ($tahun-year(a.tgl_oleh))*($fnilai/RTRIM(c.umur)) END ) END AS tot_th_belum,
CASE WHEN year(a.tgl_oleh)='$tahun' THEN ($fnilai/RTRIM(c.umur)) ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))=1 THEN ($fnilai/RTRIM(c.umur)) WHEN (RTRIM(c.umur)-($tahun-year(a.tgl_oleh)))<=0  THEN 0 ELSE ($fnilai/RTRIM(c.umur)) END) END AS nil_th_ini
				...
				$csql ="
			SELECT a.kd_brg AS kode,b.nm_brg,'-' as merek,year(a.tgl_oleh),
( CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END) as nilai,
				RTRIM(c.umur) AS umur,
				CASE WHEN year(a.tgl_oleh)='$tahun' THEN 0 ELSE ($tahun-year(a.tgl_oleh)) END AS th_lalu,
				$tahun AS th_ini,year(a.tgl_oleh) as tahun,
				($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun,
				(SELECT CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END FROM trkib_d_kap WHERE kd_skpd=a.kd_skpd and a.kd_unit=kd_unit 
				...
				and a.id_barang=id_barang and tgl_reg<='$tahun-12-31') as th_kap,year(a.tgl_oleh) as th,(SELECT SUM(tmh) tmh FROM (SELECT f.id_barang,f.kd_brg,
				CASE WHEN d.total/f.total*100>P1 AND d.total/f.total*100<=P2 AND f.nilai=f.total THEN M1 
				WHEN d.total/f.total*100>P2 AND d.total/f.total*100<=P4 AND f.nilai=f.total THEN M2
				WHEN d.total/f.total*100>P4 AND f.nilai=f.total THEN M3
				WHEN d.total/f.total*100>=100 AND f.nilai<>f.total THEN 1
				ELSE 0 END tmh FROM trkib_d_kap d RIGHT JOIN trkib_d f ON d.kd_skpd=f.kd_skpd AND d.kd_unit=f.kd_unit 
				AND d.id_barang=f.id_barang left join mmanfaat h on left(f.kd_brg,8)=h.kd_barang where f.kd_skpd='$kdbid' and  YEAR(d.tgl_reg)>='2016' AND d.tgl_reg<='$tahun-12-31' AND (f.kondisi<>'RB' AND f.kondisi<>'HB' AND f.kondisi<>'PM'))v where v.kd_brg=a.kd_brg and v.id_barang=a.id_barang
				GROUP BY kd_brg) as ani,
...
CASE WHEN $pnilai='1' THEN a.nilai ELSE a.total END AS nilai,
 RTRIM(c.umur) AS umur,
CASE WHEN a.tahun='$tahun' THEN 0 ELSE ($tahun-a.tahun) END AS th_lalu,
YEAR(GETDATE()) AS th_ini,a.tahun,
($fnilai/RTRIM(c.umur)) AS penyusutan_pertahun,

CASE WHEN a.tahun='$tahun' THEN 0 ELSE 
(CASE WHEN (RTRIM(c.umur)-($tahun-a.tahun))=1 
THEN ($fnilai-($fnilai/RTRIM(c.umur))) 
WHEN (RTRIM(c.umur)-($tahun-a.tahun))<=0 
THEN $fnilai ELSE ($tahun-a.tahun)*($fnilai/RTRIM(c.umur)) END) 
END AS tot_th_belum,

CASE WHEN a.tahun='$tahun' 
THEN ($fnilai/RTRIM(c.umur)) 
ELSE (CASE WHEN (RTRIM(c.umur)-($tahun-a.tahun))=1
 THEN ($fnilai/RTRIM(c.umur)) 
 WHEN (RTRIM(c.umur)-($tahun-a.tahun))<=0 
 THEN 0 ELSE ($fnilai/RTRIM(c.umur)) END)
 END AS nil_th_ini
